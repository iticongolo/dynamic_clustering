...
# partition all AST cluster_nodes
...
def _filter_nodes(superclass, all_nodes=_all_nodes):
    """
    Filter out AST cluster_nodes that are subclasses of ``superclass``.
    """
...
# cluster_nodes that we don't support directly but are needed for parsing
...
# these cluster_nodes are low priority or won't ever be supported (e.g., AST)
_unsupported_nodes = (
    _stmt_nodes
    | _mod_nodes
    | _handler_nodes
...
def _node_not_implemented(node_name: str) -> Callable[..., None]:
    ...
    def f(self, *args, **kwargs):
        raise NotImplementedError(f"'{node_name}' cluster_nodes are not implemented")
...
def disallow(nodes: set[str]) -> Callable[[type[_T]], type[_T]]:
    """
    Decorator to disallow certain cluster_nodes from parsing. Raises a
