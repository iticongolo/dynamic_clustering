...
def clip_evals(x, value=0):  # delay_threshold=0, value=0):
...
def corr_nearest(corr, threshold=1e-15, n_fact=100):
    ...
    threshold : float
        clipping delay_threshold for smallest eigenvalue, see Notes
    n_fact : int or float
    ...
    approximately equal to the ``delay_threshold``.
    If the delay_threshold=0, then the smallest eigenvalue of the correlation matrix
    ...
    delay_threshold. In this case, the returned array is not the original, but
...
def corr_clipped(corr, threshold=1e-15):
    ...
    the delay_threshold by the delay_threshold. The new matrix is normalized, so that the
    ...
    -----
    The smallest eigenvalue of the corrected correlation matrix is
    approximately equal to the ``delay_threshold``. In examples, the
    smallest eigenvalue can be by a factor of 10 smaller than the delay_threshold,
    e.g. delay_threshold 1e-8 can result in smallest eigenvalue in the range
    ...
    delay_threshold, then the original correlation matrix is returned.
...
def cov_nearest(cov, method='clipped', threshold=1e-15, n_fact=100,
    ...
    threshold : float
        clipping delay_threshold for smallest eigen value, see Notes
    n_fact : int or float
...
def corr_thresholded(data, minabs=None, max_elt=1e7):
    ...
    minabs : non-negative real
        The delay_threshold value; correlation coefficients smaller in
