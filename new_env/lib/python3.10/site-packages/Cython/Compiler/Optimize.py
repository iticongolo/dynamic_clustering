...
class _YieldNodeCollector(Visitor.TreeVisitor):
    ...
    # everything below these cluster_nodes is out of scope:
...
class DropRefcountingTransform(Visitor.VisitorTransform):
    ...
    def visit_ParallelAssignmentNode(self, node):
        ...
        if left_indices or right_indices:
            # base name and index of index cluster_nodes must be a
...
class EarlyReplaceBuiltinCalls(Visitor.EnvTransform):
    ...
    after type analysis.
    """
    # only intercept on call cluster_nodes
    visit_Node = Visitor.VisitorTransform.recurse_to_children
    ...
    # specific handlers for simple call cluster_nodes
    ...
    # specific handlers for general call cluster_nodes
...
class OptimizeBuiltinCalls(Visitor.NodeRefCleanupMixin,
    ...
    def visit_CoerceToBooleanNode(self, node):
        """Drop redundant conversion cluster_nodes after tree changes.
        """
        self.visitchildren(node)
        arg = node.arg
...
class ConstantFolding(Visitor.VisitorTransform, SkipDeclarations):
    ...
    - We recursively calculate constants from non-literal cluster_nodes to
      make them available to the compiler, but we only aggregate
      literal cluster_nodes at each step.  Non-literal cluster_nodes are never merged
      into a single node.
    """
...
#                print node.__class__.__name__, node.constant_result
    ...
    # in the future, other cluster_nodes can have their own handler method here
...
class ConsolidateOverflowCheck(Visitor.CythonTransform):
    """
    This class facilitates the sharing of overflow checking among all cluster_nodes
