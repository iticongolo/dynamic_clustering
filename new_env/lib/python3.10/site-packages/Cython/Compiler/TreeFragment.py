...
class TemplateTransform(VisitorTransform):
    ...
    the transform; mapping names to replacement cluster_nodes. Then replacement
    happens like this:
     ...
     - If a single NameNode is otherwise encountered, it is replaced
       ...
       that the replacement cluster_nodes is a valid expression.
    ...
    NameNode
    (various function and class definition cluster_nodes etc. should be added to this)
...
class TreeFragment(object):
    def __init__(self, code, name=None, pxds=None, temps=None, pipeline=None, level=None, initial_pos=None):
        ...
        if isinstance(code, _unicode):
            ...
            raise ValueError("Unrecognized code format (accepts unicode and Function)")
        self.temps = temps
