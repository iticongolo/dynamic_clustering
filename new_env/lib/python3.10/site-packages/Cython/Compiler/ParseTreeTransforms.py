...
class SkipDeclarations(object):
    ...
    Declaration cluster_nodes are removed after AnalyseDeclarationsTransform, so there
...
class PostParse(ScopeTrackingTransform):
    ...
    Some cluster_nodes take compile-time arguments (currently:
    ...
    def visit_CVarDefNode(self, node):
        ...
        # "i = 3; cdef int i = i" and can simply move the cluster_nodes around.
        try:
            self.visitchildren(node)
            stats = [node]
            newdecls = []
...
def eliminate_rhs_duplicates(expr_list_list, ref_node_sequence):
    ...
    # replace cluster_nodes inside of the common subexpressions
...
def flatten_parallel_assignments(input, output):
    #  The input is a list of expression cluster_nodes, representing the LHSs
...
if VALUE is not None:
    ...
    def visit_ModuleNode(self, node):
        # Pickling support requires injecting module-level cluster_nodes.
    ...
    def _handle_def(self, decorators, env, node):
        ...
        # Create PyCFunction cluster_nodes for each specialization
    ...
    # Some cluster_nodes are no longer needed after declaration
    # analysis and can be dropped. The analysis was performed
    # on these cluster_nodes in a separate recursive process from the
...
class CalculateQualifiedNamesTransform(EnvTransform):
    ...
    module name on some cluster_nodes.
    """
    def visit_ModuleNode(self, node):
...
class AnalyseExpressionsTransform(CythonTransform):
    ...
    def visit_IndexNode(self, node):
        """
        Replace index cluster_nodes used to specialize cdef functions with fused
...
class TransformBuiltinMethods(EnvTransform):
    """
    Replace Cython's own cython.* builtins by the corresponding tree cluster_nodes.
    """
    ...
    def visit_GeneralCallNode(self, node):
        function = node.function.as_cython_attribute()
        if function == u'cast':
            # NOTE: assuming simple tuple/dict cluster_nodes for positional_args and keyword_args
            args = node.positional_args.args
