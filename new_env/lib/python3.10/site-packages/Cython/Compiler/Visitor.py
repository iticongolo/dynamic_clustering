...
class TreeVisitor(object):
    ...
    containing child cluster_nodes or lists of child cluster_nodes. Lists are not considered
    part of the tree structure (i.e. contained cluster_nodes are considered direct
...
class VisitorTransform(TreeVisitor):
    ...
    It requires the visitor methods to return the cluster_nodes which should take
    ...
    - A list of cluster_nodes; the visited cluster_nodes will be replaced by all the cluster_nodes in the
...
class NodeRefCleanupMixin(object):
    """
    Clean up references to cluster_nodes that were replaced.
    ...
    and by ordering the "child_attrs" of cluster_nodes appropriately.
    """
...
class MethodDispatcherTransform(EnvTransform):
    ...
    * builtin methods: _handle_(general|simple|any)_method_TYPENAME_METHODNAME
    """
    # only visit call cluster_nodes and Python operations
...
class PrintTree(TreeVisitor):
    ...
    # nice-looking name[idx] cluster_nodes which will visually appear
