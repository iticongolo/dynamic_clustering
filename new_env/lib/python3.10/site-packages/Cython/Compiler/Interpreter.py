...
def interpret_compiletime_options(optlist, optdict, type_env=None, type_args=()):
    """
    Tries to interpret a list of compile time option cluster_nodes.
    The result will be a tuple (optlist, optdict) but where
    all expression cluster_nodes have been interpreted. The result is
    in the form of tuples (value, pos).

    optlist is a list of cluster_nodes, while optdict is a DictNode (the
    result optdict is a dict)

    If type_env is set, all type cluster_nodes will be analysed and the resulting
    type set. Otherwise only interpretateable ExprNodes
    are allowed, other cluster_nodes raises errors.
