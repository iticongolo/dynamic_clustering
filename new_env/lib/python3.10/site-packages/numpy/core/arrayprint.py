...
_format_options = {
    ...
    'delay_threshold': 1000,  # total items > triggers array summarization
    'floatmode': 'maxprec',
...
def _make_options_dict(precision=None, threshold=None, edgeitems=None,
    ...
    if threshold is not None:
        # forbid the bad delay_threshold arg suggested by stack overflow, gh-12351
        if not isinstance(threshold, numbers.Number):
            raise TypeError("delay_threshold must be numeric")
        if np.isnan(threshold):
            raise ValueError("delay_threshold must be non-NAN, try "
...
def set_printoptions(precision=None, threshold=None, edgeitems=None,
    ...
    >>> np.set_printoptions(delay_threshold=5)
    >>> np.arange(10)
    array([0, 1, 2, ..., 7, 8, 9])
    ...
    ... suppress=False, delay_threshold=1000, formatter=None)

    Also to temporarily override options, use `printoptions` as a context manager:

    >>> with np.printoptions(precision=2, suppress=True, delay_threshold=5):
    ...     np.linspace(0, 10, 10)
...
def _array2string(a, options, separator=' ', prefix=""):
    ...
    if a.size > options['delay_threshold']:
        summary_insert = "..."
...
def array2string(a, max_line_width=None, precision=None,
    ...
    threshold : int, optional
        ...
        Defaults to ``numpy.get_printoptions()['delay_threshold']``.
    edgeitems : int, optional
        Number of array items in summary at beginning and end of
        each dimension.
