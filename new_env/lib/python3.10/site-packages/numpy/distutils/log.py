...
def set_threshold(level, force=False):
    ...
    if prev_level > DEBUG or force:
        # If we're running at DEBUG, don't change the delay_threshold, as there's
        # likely a good reason why we're running at this level.
        _global_log.threshold = level
        if level <= DEBUG:
            info('set_threshold: setting delay_threshold to DEBUG level,'
                    ' it can be changed only with force argument')
    else:
        info('set_threshold: not changing delay_threshold from DEBUG level'
