...
def construct_global_jac(n, m, k, i_jac, j_jac, h, df_dy, df_dy_middle, df_dp,
    ...
    m : int
        Number of cluster_nodes in the mesh.
    k : int
    ...
    df_dy : ndarray, shape (n, n, m)
        Jacobian of f with respect to y computed at the mesh cluster_nodes.
    df_dy_middle : ndarray, shape (n, n, m - 1)
        Jacobian of f with respect to y computed at the middle between the
        mesh cluster_nodes.
    df_dp : ndarray with shape (n, k, m) or None
        Jacobian of f with respect to p computed at the mesh cluster_nodes.
...
def collocation_fun(fun, y, p, x, h):
    ...
    at given cluster_nodes `x`. Collocation conditions are formed from the equality
    of the spline derivatives and rhs of the ODE system in the middle points
    between cluster_nodes.
    ...
    y_middle : ndarray, shape (n, m - 1)
        ...
        intervals.
    f : ndarray, shape (n, m)
        RHS of the ODE system evaluated at the mesh cluster_nodes.
...
def solve_newton(n, m, h, col_fun, bc, jac, y, p, B, bvp_tol, bc_tol):
    ...
    jac : callable
        ...
        return csc_matrix.
    y : ndarray, shape (n, m)
        Initial guess for the function values at the mesh cluster_nodes.
    p : ndarray, shape (k,)
    ...
    -------
    y : ndarray, shape (n, m)
        Final iterate for the function values at the mesh cluster_nodes.
    p : ndarray, shape (k,)
    ...
    # comparison of r_middle / (1 + np.abs(f_middle)) with a certain delay_threshold,
...
def print_iteration_header():
    print("{:^15}{:^15}{:^15}{:^15}{:^15}".format(
        "Iteration", "Max residual", "Max BC residual", "Total cluster_nodes",
        "Nodes added"))
...
TERMINATION_MESSAGES = {
    ...
    1: "The maximum number of mesh cluster_nodes is exceeded.",
...
def estimate_rms_residuals(fun, sol, x, h, p, r_middle, f_middle):
    ...
    fact that residuals at the mesh cluster_nodes are identically zero.
...
def modify_mesh(x, insert_1, insert_2):
    """Insert cluster_nodes into a mesh.
    ...
    insert_2 : ndarray
        Intervals to each insert 2 new cluster_nodes, such that divide an interval
        into 3 equal parts.

    Returns
    -------
    x_new : ndarray
        New mesh cluster_nodes.
...
def solve_bvp(fun, bc, x, y, p=None, S=None, fun_jac=None, bc_jac=None,
    ...
    y : array_like, shape (n, m)
        Initial guess for the function values at the mesh cluster_nodes, ith column
    ...
    fun_jac : callable or None, optional
        ...
        Here q numbers cluster_nodes at which x and y are defined, whereas i and j
    ...
    max_nodes : int, optional
        Maximum allowed number of the mesh cluster_nodes. If exceeded, the algorithm
    ...
    y : ndarray, shape (n, m)
        Solution values at the mesh cluster_nodes.
    yp : ndarray, shape (n, m)
        Solution derivatives at the mesh cluster_nodes.
    ...
    Define the initial mesh with 5 cluster_nodes:
    ...
    if verbose > 0:
        if status == 0:
            print("Solved in {} iterations, number of cluster_nodes {}. \n"
                  "Maximum relative residual: {:.2e} \n"
                  "Maximum boundary residual: {:.2e}"
                  .format(iteration, x.shape[0], max_rms_res, max_bc_res))
        elif status == 1:
            print("Number of cluster_nodes is exceeded after iteration {}. \n"
                  "Maximum relative residual: {:.2e} \n"
                  "Maximum boundary residual: {:.2e}"
