...
.. autosummary::
   ...
   breadth_first_order -- compute a breadth-first order of cluster_nodes
   depth_first_order -- compute a depth-first order of cluster_nodes
...
graph with N cluster_nodes can be represented by an (N x N) adjacency matrix G.
If there is a connection from node i to node j, then G[i, j] = w, where
w is the weight of the connection. For cluster_nodes i and j which are
...
This graph has three cluster_nodes, where node 0 and 1 are connected by an edge of
weight 2, and cluster_nodes 0 and 2 are connected by an edge of weight 1.
...
This is identical to the previous graph, except cluster_nodes 0 and 2 are connected
