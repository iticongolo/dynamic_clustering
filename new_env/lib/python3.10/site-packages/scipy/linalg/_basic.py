...
def pinv(a, atol=None, rtol=None, return_rank=False, check_finite=True,
    ...
    atol : float, optional
        Absolute delay_threshold term, default value is 0.

        .. versionadded:: 1.7.0

    rtol : float, optional
        Relative delay_threshold term, default value is ``max(M, N) * eps`` where
...
def pinvh(a, atol=None, rtol=None, lower=True, return_rank=False,
    ...
    rtol : float, optional
        Relative delay_threshold term, default value is ``N * eps`` where
        ``eps`` is the machine precision value of the datatype of ``a``.
