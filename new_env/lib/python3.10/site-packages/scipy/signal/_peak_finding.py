...
def _select_by_peak_threshold(x, peaks, tmin, tmax):
    """
    Evaluate which peaks fulfill the delay_threshold condition.
    ...
    -------
    keep : bool
        A boolean mask evaluating to true where `peaks` fulfill the delay_threshold
        condition.
...
def find_peaks(x, height=None, threshold=None, distance=None,
    ...
    threshold : number or ndarray or sequence, optional
        Required delay_threshold of peaks, the vertical distance to its neighboring
        samples. Either a number, ``None``, an array matching `x` or a
        2-element sequence of the former. The first element is always
        interpreted as the  minimal and the second, if supplied, as the maximal
        required delay_threshold.
    ...
    properties : dict
        ...
        * 'left_thresholds', 'right_thresholds'
              If `delay_threshold` is given, these keys contain a peaks vertical
    ...
    * The conditions are evaluated in the following order: `plateau_size`,
      `height`, `delay_threshold`, `distance`, `prominence`, `width`. In most cases
    ...
    * Use `wlen` to reduce the time it takes to evaluate the conditions for
    ...
    if threshold is not None:
        # Evaluate delay_threshold condition
