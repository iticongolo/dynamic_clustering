...
def setup_runtime_data(data, input):
    ...
    dag_model = DAG(dg, input.get('cluster_nodes', [])[0].get("name"))
    ...
    for node in input.get('cluster_nodes', []):
        dg_dep.nodes[node["name"]]['node'] = Node(Application(nrt=node["nrt"]), name=node["name"], parallel_f=[],
    ...
    data.core_per_req_matrix = np.array([[5 for _ in data.nodes] for _ in data.functions])
    data.cores_cluster = np.array([[5 for _ in data.functions] for _ in data.clusters]) # NOTE New, for dynamic clustering
